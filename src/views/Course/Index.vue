        ...
        <div class="course-box-tab">
          <div class="course-tab-item">
            <div @click="courstTab(1)">
              <p :style="{ color: courseTab == 1 ? 'rgba(0, 0, 0, 0.8)' : '' }">
                智慧早教
              </p>
              <span v-if="courseTab == 1"></span>
            </div>
            <div @click="courstTab(2)">
              <p :style="{ color: courseTab == 2 ? 'rgba(0, 0, 0, 0.8)' : '' }">
                我的课程
              </p>
...
export default {
  ...
  methods: {
    ...
    async getCourseAll() {
      try {
        const babyId = this.babyid || localStorage.getItem('courseBaby')

        this.$axios
          .getCoursePack(babyId)
          .then((res) => {
            if (res.data.code == 1) {
